...
// creeating an implentation of the interface
...
func (u *UserServiceImpl) GetAllUsers() ([]*models.User, error) {
	...
	// if no error, then we iterate over all users by passing the context to the next method
	...
	cursor.Close(u.ctx)
	if len(users) == 0 {
		return nil, errors.New("the cursor is empty && document is empty")
	}
